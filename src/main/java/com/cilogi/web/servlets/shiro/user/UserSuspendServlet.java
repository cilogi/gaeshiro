...
package com.cilogi.web.servlets.shiro.user;
...
import com.cilogi.web.servlets.shiro.Names;
...
public class UserSuspendServlet extends BaseServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            String userName = request.getParameter(Names.USERNAME);
            GaeUser user = (GaeUser)gaeUserDAO.findUser(userName);
            if (user != null) {
                boolean isSuspend = Boolean.parseBoolean(request.getParameter(Names.SUSPEND));
                boolean isDelete = Boolean.parseBoolean(request.getParameter(Names.DELETE));
                if (isDelete) {
                ...
                issue(MimeTypes.MIME_TEXT_PLAIN, HTTP_STATUS_NOT_FOUND,
                      "Can't find user " + userName, response);
            }
        } catch (Exception e) {
            log.severe("Suspend failure: " + e.getMessage());
            issue(MimeTypes.MIME_TEXT_PLAIN, HTTP_STATUS_INTERNAL_SERVER_ERROR,
